...
// Dropdown is position:fixed relative to the viewport, so without this it
// would stay frozen in place while the page scrolls underneath it, making
// it look "detached" from its category button. Instead of closing it on
// scroll, keep it open and recompute its position on every scroll frame so
// it visually stays attached to the button. rAF-throttled to avoid firing
// the position math on every single scroll event.
document.addEventListener("DOMContentLoaded", () => {
  const nav = document.getElementById("sa-category-nav");
  let ticking = false;

  const repositionOpenDropdown = () => {
    const openDropdown = document.querySelector(".sa-cat-dropdown.open");
    if (!openDropdown) return;
    const catId = openDropdown.id.replace("sa-cat-dropdown-", "");
    const item = document.getElementById(`sa-cat-item-${catId}`);
    if (item) positionCategoryDropdown(item, openDropdown);
  };
  ...
  // Page scroll (vertical) AND the nav's own horizontal scroll both need
  // to move the dropdown, since the button can shift in either direction.
  window.addEventListener("scroll", onScroll, { passive: true });
  if (nav) nav.addEventListener("scroll", onScroll, { passive: true });

  // Resize can change layout enough (e.g. rotating a phone) that a stale
  // position would be actively wrong rather than just off by a few px, so
  // this one still just closes the dropdown instead of repositioning.
